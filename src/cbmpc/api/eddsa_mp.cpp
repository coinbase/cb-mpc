...
error_t dkg_additive(const job_mp_t& job, curve_id curve, buf_t& key_blob, buf_t& sid) {
  ...
  sid.free();
  key_blob.free();
  rv = coinbase::mpc::schnorrmp::dkg(mpc_job, coinbase::crypto::curve_ed25519, key, sid);
  if (rv) return rv;
...
error_t dkg_ac(const job_mp_t& job, curve_id curve, buf_t& sid, const access_structure_t& access_structure,
  ...
  rv = coinbase::api::detail::to_internal_access_structure(access_structure, job.party_names,
                                                           coinbase::crypto::curve_ed25519, ac);
  if (rv) return rv;
  ...
  coinbase::mpc::schnorrmp::key_t key;
  key_blob.free();
  rv = coinbase::mpc::schnorrmp::dkg_ac(mpc_job, coinbase::crypto::curve_ed25519, sid, ac, quorum_party_set, key);
  if (rv) return rv;
