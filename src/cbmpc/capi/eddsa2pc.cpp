...
cbmpc_error_t cbmpc_eddsa_2p_dkg(const cbmpc_2pc_job_t* job, cbmpc_curve_id_t curve, cmem_t* out_key_blob) {
  try {
    ...
    *out_key_blob = cmem_t{nullptr, 0};
    ...
    const coinbase::error_t rv = coinbase::api::eddsa_2p::dkg(ctx.job, curve_cpp, key_blob);
    if (rv) return rv;
...
cbmpc_error_t cbmpc_eddsa_2p_refresh(const cbmpc_2pc_job_t* job, cmem_t key_blob, cmem_t* out_new_key_blob) {
  try {
    ...
    *out_new_key_blob = cmem_t{nullptr, 0};
    ...
    const coinbase::error_t rv = coinbase::api::eddsa_2p::refresh(ctx.job, view_cmem(key_blob), new_key);
    if (rv) return rv;
...
cbmpc_error_t cbmpc_eddsa_2p_sign(const cbmpc_2pc_job_t* job, cmem_t key_blob, cmem_t msg, cmem_t* sig_out) {
  try {
    ...
    *sig_out = cmem_t{nullptr, 0};
    ...
    job_2p_cpp_ctx_t ctx(job, self_cpp);
    coinbase::buf_t sig;
    const coinbase::error_t rv = coinbase::api::eddsa_2p::sign(ctx.job, view_cmem(key_blob), view_cmem(msg), sig);
    if (rv) return rv;
...
cbmpc_error_t cbmpc_eddsa_2p_get_public_key_compressed(cmem_t key_blob, cmem_t* out_pub_key) {
  try {
    ...
    *out_pub_key = cmem_t{nullptr, 0};
    ...
    coinbase::buf_t pk;
    const coinbase::error_t rv = coinbase::api::eddsa_2p::get_public_key_compressed(view_cmem(key_blob), pk);
    if (rv) return rv;
...
cbmpc_error_t cbmpc_eddsa_2p_get_public_share_compressed(cmem_t key_blob, cmem_t* out_public_share) {
  try {
    ...
    *out_public_share = cmem_t{nullptr, 0};
    ...
    coinbase::buf_t Qi;
    const coinbase::error_t rv = coinbase::api::eddsa_2p::get_public_share_compressed(view_cmem(key_blob), Qi);
    if (rv) return rv;
  ...
  try {
    ...
    *out_private_scalar_fixed = cmem_t{nullptr, 0};
    ...
    const coinbase::error_t rv =
        coinbase::api::eddsa_2p::detach_private_scalar(view_cmem(key_blob), public_blob, private_scalar_fixed);
    if (rv) return rv;
