  ...
  try {
    ...
    *out_share = cmem_t{nullptr, 0};
    ...
    if (base_pke) {
      c_base_pke_adapter_t adapter(base_pke);
      rv =
          coinbase::api::pve::party_decrypt_row_ac(adapter, curve_cpp, ac_cpp, view_cmem(ciphertext), row_index,
                                                   std::string_view(leaf_name), view_cmem(dk), view_cmem(label), share);
    } else {
      rv =
          coinbase::api::pve::party_decrypt_row_ac(curve_cpp, ac_cpp, view_cmem(ciphertext), row_index,
                                                   std::string_view(leaf_name), view_cmem(dk), view_cmem(label), share);
    }
    if (rv) return rv;
...
cbmpc_error_t cbmpc_pve_ac_party_decrypt_row_rsa_oaep_hsm(cbmpc_curve_id_t curve, const cbmpc_access_structure_t* ac,
  ...
  try {
    ...
    *out_share = cmem_t{nullptr, 0};
    ...
    coinbase::buf_t share;
    const coinbase::error_t rv = coinbase::api::pve::party_decrypt_row_ac_rsa_oaep_hsm(
        curve_cpp, ac_cpp, view_cmem(ciphertext), row_index, std::string_view(leaf_name), view_cmem(dk_handle),
...
cbmpc_error_t cbmpc_pve_ac_party_decrypt_row_ecies_p256_hsm(cbmpc_curve_id_t curve, const cbmpc_access_structure_t* ac,
  ...
  try {
    ...
    *out_share = cmem_t{nullptr, 0};
    ...
    coinbase::buf_t share;
    const coinbase::error_t rv = coinbase::api::pve::party_decrypt_row_ac_ecies_p256_hsm(
        curve_cpp, ac_cpp, view_cmem(ciphertext), row_index, std::string_view(leaf_name), view_cmem(dk_handle),
        view_cmem(ek), view_cmem(label), cb_cpp, share);
    if (rv) return rv;
...
cbmpc_error_t cbmpc_pve_ac_aggregate_to_restore_row(const cbmpc_pve_base_pke_t* base_pke, cbmpc_curve_id_t curve,
  ...
  try {
    ...
    *out_xs = cmems_t{0, nullptr, nullptr};
    ...
    if (base_pke) {
      c_base_pke_adapter_t adapter(base_pke);
      rv = coinbase::api::pve::aggregate_to_restore_row_ac(adapter, curve_cpp, ac_cpp, view_cmem(ciphertext), row_index,
                                                           view_cmem(label), quorum_cpp, xs_cpp);
    } else {
      rv = coinbase::api::pve::aggregate_to_restore_row_ac(curve_cpp, ac_cpp, view_cmem(ciphertext), row_index,
                                                           view_cmem(label), quorum_cpp, xs_cpp);
    }
    if (rv) return rv;
...
cbmpc_error_t cbmpc_pve_ac_get_Qs(cmem_t ciphertext, cmems_t* out_Qs_compressed) {
  try {
    ...
    *out_Qs_compressed = cmems_t{0, nullptr, nullptr};
    ...
    const coinbase::error_t rv = coinbase::api::pve::get_Qs_ac(view_cmem(ciphertext), Qs);
    if (rv) return rv;
