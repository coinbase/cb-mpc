...
class ac_t {
 public:
  explicit ac_t() = default;
  explicit ac_t(const node_t *_root) : root(_root) {}
  ac_t(const node_t *_root, ecurve_t _curve) : root(_root), curve(_curve) {}
  explicit ac_t(ecurve_t _curve) : curve(_curve) {}

  const node_t *get_root() const { return root; }
  bool has_root() const { return root != nullptr; }
  ecurve_t get_curve() const { return curve; }
  bool has_curve() const { return curve.valid(); }
...
class ac_owned_t : public ac_t {
 public:
  ...
  explicit ac_owned_t(const node_t *_root, ecurve_t _curve) { assign(_root, _curve); }
  explicit ac_owned_t(const ac_t &ac) { assign(ac); }
  ~ac_owned_t() { delete root; }
  void assign(const node_t *_root) {
    delete root;
    root = _root->clone();
  }
  void assign(const node_t *_root, ecurve_t _curve) {
    curve = _curve;
    assign(_root);
  }
  void assign(const ac_t &ac) {
    curve = ac.curve;
    assign(ac.root);
  }
  ac_owned_t(const ac_owned_t &src) : ac_t() { assign(static_cast<const ac_t &>(src)); }
  ...
  ac_owned_t &operator=(const ac_owned_t &src) {
    if (&src != this) assign(static_cast<const ac_t &>(src));
    return *this;
  }
