...
class paillier_t {
 public:
  ...
  class rerand_scope_t {
   public:
    ...
    rerand_scope_t(const rerand_scope_t&) = delete;
    rerand_scope_t& operator=(const rerand_scope_t&) = delete;
    rerand_scope_t(rerand_scope_t&&) = delete;
    rerand_scope_t& operator=(rerand_scope_t&&) = delete;
  ...
  error_t verify_ciphers(Values... ciphers) const {
    ...
    return batch_verify_ciphers(arr.data(), static_cast<int>(arr.size()));
  }
 ...
 private:
  ...
  // Cached inverse of N modulo 2^bit_size for constant-time extraction of L(u) = (u-1)/N
  // during Paillier decryption (avoid variable-time big integer division on secret-dependent values).
  bn_t inv_N_mod_2k;
