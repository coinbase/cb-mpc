...
class bn_t {
 ...
 public:
  ...
  void to_bin(mem_t mem) const { to_bin(const_cast<byte_ptr>(mem.data), mem.size); }
  static bn_t from_bin(mem_t mem);
  static error_t vector_from_bin(mem_t mem, int n, int size, const mod_t& q, std::vector<bn_t>& out);
  ...
  static error_t from_string(const_char_ptr str, bn_t& result);
  static error_t from_string(const std::string& str, bn_t& result) { return from_string(str.c_str(), result); }
  static error_t from_hex(const_char_ptr str, bn_t& result);
  ...
  // Maximum number of bytes allowed when (de)serializing a `bn_t`.
  //
  // This is a defense-in-depth bound against attacker-controlled inputs causing excessive allocations or crashes
  // during deserialization. Override at build time by defining `CBMPC_MAX_SERIALIZED_BIGNUM_BYTES`.
  inline static constexpr uint32_t MAX_SERIALIZED_BIGNUM_BYTES =
#if defined(CBMPC_MAX_SERIALIZED_BIGNUM_BYTES)
      CBMPC_MAX_SERIALIZED_BIGNUM_BYTES;
#else
      1024 * 1024;  // 1 MiB
#endif
