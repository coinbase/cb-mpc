...
class converter_t {
 public:
  ...
  // Maximum value allowed for `convert_len(...)` when deserializing.
  //
  // `convert_len` is used as a length prefix for buffers and container counts. Bounding it protects against
  // attacker-controlled allocations and loops if a malicious peer supplies an oversized length prefix.
  static constexpr uint32_t MAX_CONVERT_LEN = 64 * 1024 * 1024;  // 64 MiB
