...
struct key_share_mp_t {
 ...
 public:
   ...
   * @notes:
   * - This access-structure DKG is not optimal in the sense that all n parties need to be connected
   ...
   * In the future, we may add primitives that make it easier to implement a DKG where only a subset of the
 ...
 private:
  static error_t dkg_or_refresh_ac(job_mp_t& job, const ecurve_t& curve, buf_t& sid, const crypto::ss::ac_t,
                                   const party_set_t& quorum_party_set, key_share_mp_t& key, key_share_mp_t& new_key,
                                   bool is_refresh);
};
