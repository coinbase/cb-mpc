...
namespace coinbase {
// `SUCCESS` is always zero. All values produced by `ERRCODE(...)` are non-zero
// error returns and should be checked with `rv != SUCCESS` (or simply `if (rv)`),
// never with `rv > 0`.
//
// The encoding intentionally uses the `0xff......` range for non-zero errors.
// On our supported platforms these values compare as negative when stored in the
// signed `error_t`, which is expected and relied on throughout the codebase.
using error_t = int;
}  // namespace coinbase

// Encodes a non-zero error value with the category in bits 16..23.
