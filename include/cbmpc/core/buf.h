...
inline void bzero(byte_ptr pointer, int size) {
  cb_assert(size >= 0 && "bzero: negative size");
  if (size == 0) return;
  cb_assert(pointer && "bzero: null pointer");
  memset(pointer, 0, static_cast<size_t>(size));
}

inline void secure_bzero(byte_ptr pointer, int size) {
  if (size <= 0) return;
  cb_assert(pointer && "secure_bzero: null pointer");
#if defined(__STDC_LIB_EXT1__)
  // `memset_s` is guaranteed to perform the memory write and not be optimized away.
  (void)memset_s(pointer, static_cast<rsize_t>(size), 0, static_cast<rsize_t>(size));
#else
  // Best-effort fallback that compilers cannot elide.
  volatile byte_t* p = pointer;
...
struct mem_t {
  ...
  mem_t(const_byte_ptr the_data, int the_size) noexcept(true) : data(the_data), size(the_size) {}
  mem_t(const std::string& s) noexcept(true) : data(const_byte_ptr(s.data())), size(int(s.size())) {
    cb_assert(s.size() <= INT_MAX);
  }
  template <size_t N>
  mem_t(const char (&s)[N]) : data(const_byte_ptr(s)), size(N) {
    if (N > 0 && s[N - 1] == '\0') size--;  // zero-terminated
  }

  // NOTE: `mem_t` is often used as a read-only view. These mutating helpers are available for
  // cases where the underlying memory is known to be writable.
  void bzero() { coinbase::bzero(const_cast<byte_ptr>(data), size); }
  void secure_bzero() { coinbase::secure_bzero(const_cast<byte_ptr>(data), size); }
  void reverse();
  buf_t rev() const;
  ...
  uint8_t& operator[](int index) {
    cb_assert(index >= 0 && index < size);
    return const_cast<byte_ptr>(data)[index];
  }
...
class buf_t {
 public:
  buf_t() noexcept(true);
  // NOTE: Allocates storage but intentionally does not initialize it.
  // Callers must fully overwrite the buffer before reading from `data()`.
  ...
  // NOTE: Like `buf_t(int)`, `alloc()` leaves the returned buffer contents uninitialized.
  // Callers must fully overwrite the buffer before reading from `data()`.
