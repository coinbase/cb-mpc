...
const (
	...
	ECDSAMPACResignStagePortOffset  = 300
)
...
// used by the shipped ECDSA MP AC demos.
...
func newReadyStageTransport(ctx context.Context, self int, partyAddrs []string, tlsConfig *tls.Config) (*MTLSTransport, error) {
	if self < 0 || self >= len(partyAddrs) {
		return nil, fmt.Errorf("self %d out of range [0, %d)", self, len(partyAddrs))
	}
	transport, err := NewMTLSTransport(self, len(partyAddrs), partyAddrs[self], partyAddrs, tlsConfig)
	if err != nil {
		return nil, err
	}
...
func RunECDSAMPACDKG(ctx context.Context, self int, partyNames []string, partyAddrs []string, tlsConfig *tls.Config, curve mpc.Curve, access *mpc.AccessStructure, quorumPartyNames []string) (*ECDSAMPACDKGResult, error) {
	transport, err := newReadyStageTransport(ctx, self, partyAddrs, tlsConfig)
	if err != nil {
...
func RunECDSAMPACSign(ctx context.Context, self int, quorumPartyNames []string, partyAddrs []string, tlsConfig *tls.Config, access *mpc.AccessStructure, acKeyBlob []byte, msgHash []byte, expectedPublicKey []byte, sigReceiver int) (*ECDSAMPACSignResult, error) {
	transport, err := newReadyStageTransport(ctx, self, partyAddrs, tlsConfig)
	if err != nil {
