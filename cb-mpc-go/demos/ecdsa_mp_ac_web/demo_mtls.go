...
func demoPeerNameForIndex(index int) (string, error) {
	if index < 0 {
		return "", fmt.Errorf("party index must be non-negative, got %d", index)
	}
	if index >= len(demoPeerLabels) {
		return "", fmt.Errorf("party index %d exceeds demo peer identity limit %d", index, len(demoPeerLabels))
	}
	return fmt.Sprintf("peer-%s.demo.cbmpc.local", demoPeerLabels[index]), nil
}

func demoPeerNamesForIndices(indices []int) ([]string, error) {
	names := make([]string, len(indices))
	for i, idx := range indices {
		name, err := demoPeerNameForIndex(idx)
		if err != nil {
			return nil, fmt.Errorf("partyIndices[%d]: %w", i, err)
		}
		names[i] = name
	}
	return names, nil
...
// MTLSTransport is local demo transport scaffolding kept in this module so the
// web demo behaves like a standalone application built on pkg/mpc.
...
func NewMTLSTransport(myIndex, partyCount int, listenAddr string, partyAddrs []string, tlsConfig *tls.Config) (*MTLSTransport, error) {
	...
	return NewNamedMTLSTransport(myIndex, listenAddr, partyAddrs, peerNames, tlsConfig)
}

// NewNamedMTLSTransport creates a local demo mTLS transport with explicit peer
// certificate identities ordered to match partyAddrs.
func NewNamedMTLSTransport(myIndex int, listenAddr string, partyAddrs []string, peerNames []string, tlsConfig *tls.Config) (*MTLSTransport, error) {
	if tlsConfig == nil {
		return nil, fmt.Errorf("tlsConfig cannot be nil")
	}
	partyCount := len(partyAddrs)
	if partyCount < 2 {
		return nil, fmt.Errorf("partyCount must be at least 2, got %d", partyCount)
	}
	if len(peerNames) != partyCount {
		return nil, fmt.Errorf("peerNames length mismatch: got %d want %d", len(peerNames), partyCount)
	}
	...
	session := &MTLSTransport{
		...
		peerNames:     append([]string(nil), peerNames...),
