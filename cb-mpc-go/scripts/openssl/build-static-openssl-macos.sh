...
# Prefer upstream Clang if available (matches cb-mpc recommendation) and default the
...
if ! command -v sha256sum >/dev/null 2>&1; then
  echo "ERROR: sha256sum not found (brew install coreutils)." >&2
  exit 1
fi
...
./Configure -g3 -static -DOPENSSL_THREADS no-shared \
  ...
  --prefix="$OPENSSL_PREFIX" darwin64-x86_64-cc
