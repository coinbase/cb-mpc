...
func main() {
	...
	for i := range sessions {
		...
		go func(idx int) {
			defer wg.Done()
			keyBlob, err := mpc.Schnorr2PDKG(ctx, jobs[idx], mpc.CurveSecp256k1)
			if err != nil {
	...
	msgHash := sha256.Sum256([]byte("cb-mpc-go schnorr2pc example"))
	sigs := make([][]byte, 2)
	errCh = make(chan error, 2)
	...
	for i := range sessions {
		...
		go func(idx int) {
			defer wg.Done()
			sig, err := mpc.Schnorr2PSign(ctx, jobs[idx], keyBlobs[idx], msgHash[:])
			if err != nil {
	...
	xOnly, err := mpc.Schnorr2PExtractPublicKeyXOnly(keyBlobs[0])
	if err != nil {
		fatal(err)
	}

	fmt.Printf("Schnorr-2PC sign complete\n")
	fmt.Printf("x-only pubkey bytes: %d\n", len(xOnly))
